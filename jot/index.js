...
import shallow_clone from "shallow-clone";
...
export const NO_OP = values.NO_OP;
export const SET = values.SET;
export const MATH = values.MATH;
export const PATCH = sequences.PATCH;
export const SPLICE = sequences.SPLICE;
export const ATINDEX = sequences.ATINDEX;
export const MAP = sequences.MAP;
export const PUT = objects.PUT;
export const REM = objects.REM;
export const APPLY = objects.APPLY;
export const LIST = lists.LIST;
export const COPY = copies.COPY;
/////////////////////////////////////////////////////////////////////

BaseOperation.prototype.isNoOp = function () {
  return this instanceof values.NO_OP;
}

BaseOperation.prototype.visit = function (visitor) {
...
BaseOperation.prototype.toJSON = function (__key__, protocol_version) {
...
export function opFromJSON(obj, protocol_version, op_map) {
  // Sanity check.
  ...
  if (!op_map) {
    ...
    const extend_op_map = module => {
      op_map[module.module_name] = {};
      for (let key in module.op_map)
  ...
  let dottedclassparts = obj._type.split(/\./g, 2);
  if (dottedclassparts.length !== 2) throw new Error("Not an operation.");
  let clazz = op_map[dottedclassparts[0]][dottedclassparts[1]];
...
exports.deserialize = function (op_json) {
  return opFromJSON(JSON.parse(op_json));
}

BaseOperation.prototype.compose = function (other, no_list) {
  if (!(other instanceof BaseOperation))
...
BaseOperation.prototype.rebase = function (other, conflictless, debug) {
